...
export async function POST(req: NextRequest) {
  try {
    ...
    for (const st of students) {
      if (st.parent?.fatherEmail) emailRecipients.push(st.parent.fatherEmail);
      if (st.user?.email) emailRecipients.push(st.user.email);
    }
    ...
    if (emailRecipients.length > 0) {
      const emailHtml = `
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; border: 1px solid #e2e8f0; border-radius: 16px; overflow: hidden;">
          <div style="background: linear-gradient(135deg, #ea580c, #f97316); color: white; padding: 24px; text-align: center;">
            <h2 style="margin: 0; font-size: 20px;">The Hayatabad Model School</h2>
            <p style="margin: 4px 0 0 0; font-size: 13px; opacity: 0.9;">New Homework Assignment Notification</p>
          </div>
          <div style="padding: 24px; color: #1e293b;">
            <p style="font-size: 15px; font-weight: bold; color: #ea580c; margin-top: 0;">
              Subject: ${homework.subject.name} (${homework.class.name} - ${homework.section.name})
            </p>
            <h3 style="font-size: 18px; color: #0f172a; margin: 8px 0;">${title}</h3>
            <p style="font-size: 14px; line-height: 1.6; color: #475569; background: #f8fafc; padding: 16px; border-radius: 12px; border: 1px solid #e2e8f0;">
              ${description}
            </p>
            <div style="margin-top: 16px; font-size: 13px; color: #64748b;">
              <p><strong>Teacher:</strong> ${teacherName}</p>
              <p><strong>Submission Deadline:</strong> ${new Date(dueDate).toLocaleDateString('en-GB', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' })}</p>
            </div>
            <div style="margin-top: 24px; text-align: center;">
              <a href="http://localhost:3000/login" style="display: inline-block; padding: 12px 28px; background: #ea580c; color: white; text-decoration: none; font-weight: bold; font-size: 14px; border-radius: 12px; box-shadow: 0 4px 12px rgba(234,88,12,0.3);">
                Open Student Portal
              </a>
            </div>
          </div>
      ...
      try {
        // Send email to first recipient (or test recipient) server-side
        ...
        console.warn('Homework email notification error (Resend):', err);
        emailStatus = 'FAILED';
      }
    }
